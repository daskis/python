...
def showData(con):
    try:
        ...
        cursorObj.execute("SELECT * from Routes")
        ...
        for row in rows:
            table_rows += f"""
              <tr>
                ...
                <td>{row[1]}</td>
                 <td>
                    <form method="POST" action="DeleteRoute.py">
                        <input type="hidden" name="route_id" value="{row[0]}">
        ...
        pattern = f"""<!DOCTYPE html>
            ...
            <head>
                ...
                crossorigin="anonymous"
                ></script>
                <title>Просмотр записей || Routes</title>
            </head>
            <body>
                <header>
                ...
                <main class="container h-100">
                        <div class="d-flex align-items-center justify-content-center h-100">
                            ...
                            <div class="alert alert-success" role="alert">
                                    Изменение базы данных произведено успешно
                                </div>
                            </div>
                        </div>
                        </main>
...
route_id = form.getvalue("ID_Field")
route = form.getvalue("Route_Field")

try:
    con = sqlite3.connect("AutoBase.db")
    cursorObj = con.cursor()
    cursorObj.execute("UPDATE Routes SET route=? WHERE Id=?", (route, route_id))
    con.commit()
    showData(con)
    con.close()
