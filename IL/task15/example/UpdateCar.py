...
car_id = form.getvalue("ID_Field")
brand = form.getvalue("Brand_Field")
color = form.getvalue("Color_Field")
number = form.getvalue("Number_Field")

try:
    con = sqlite3.connect("AutoBase.db")
    cursorObj = con.cursor()
    cursorObj.execute("UPDATE Cars SET Brand=?, Color=?, Number=? WHERE Id=?", (brand, color, number, car_id))
    con.commit()
    showData(con)
    con.close()
