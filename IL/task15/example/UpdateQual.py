...
def showData(con):
    try:
        ...
        cursorObj.execute("SELECT * from Qualification")
        ...
        for row in rows:
            table_rows += f"""
              <tr>
                ...
                <td>{row[2]}</td>
                 <td>
                    <form method="POST" action="DeleteQual.py">
                        <input type="hidden" name="qual_id" value="{row[0]}">
        ...
        pattern = f"""<!DOCTYPE html>
            ...
            <head>
                ...
                crossorigin="anonymous"
                ></script>
                <title>Просмотр записей || Qualification</title>
            </head>
            <body>
                <header>
                        ...
                        <div class="d-flex align-items-center justify-content-center h-100">
                            ...
                            <h1>Окно вывода данных таблицы</h1>
                             ...
                             <table class="table table-secondary table-striped table-hover">
                                ...
                                <tr>
                                    <th scope="col">Id</th>
                                    <th scope="col">Стаж вождения(В годах)</th>
                                    <th scope="col">Стаж в компании(В годах)</th>
                            ...
                            <div class="alert alert-success" role="alert">
                                    Изменение в базе данных произведено успешно
                                </div>
                            </div>
                        </div>
                        </main>
...
qual_id = form.getvalue("ID_Field")
Driving_Experience = form.getvalue("Driving_Experience_Field")
Work_Experience = form.getvalue("Work_Experience_Field")

try:
    con = sqlite3.connect("AutoBase.db")
    cursorObj = con.cursor()
    cursorObj.execute("UPDATE Qualification SET driving_exp=?, driving_in_company=? WHERE Id=?", (Driving_Experience, Work_Experience, qual_id))
    con.commit()
    showData(con)
    con.close()
    ...
    <body>
        <div class="alert alert-danger" role="alert">Произошла ошибка при изменении записи: {str(e)}</div>
    </body>
    </html>'''
    print(error_message)